...
const LoadingFallback: React.FC<{ message: string }> = ({ message }) => (
  <div className="absolute inset-0 flex items-center justify-center bg-black/30 backdrop-blur-sm z-50">
    <div className="flex items-center justify-center p-4 bg-[#1A1A2E] border-2 border-[#4A4A6A] rounded-lg shadow-xl">
      <div className="animate-spin mr-2 text-[#00D4FF]">⏳</div>
      <span className="text-[#FFECD1] font-medium">{message}</span>
    </div>
  </div>
);
...
const AppContent: React.FC = () => {
  ...
  const handleDeleteSessionRequest = useCallback((e: React.MouseEvent, id: string) => {
    e.stopPropagation();
    ...
    >
        ...
        <div className="fixed inset-0 z-[80] flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <PixelCard theme={theme} className={`w-[90%] max-w-[350px] flex flex-col gap-4 ${styles.borderColor} animate-float`}>
            ...
            <div className="flex justify-end gap-4 mt-2">
              <PixelButton theme={theme} variant="secondary" onClick={handleCloseDeleteDialog}>{t.cancel}</PixelButton>
      ...
      `}>
        ...
        <div className={`p-4 ${styles.headerBorder} space-y-2`}>
          ...
          <PixelButton theme={theme} variant="primary" className="w-full text-xs" onClick={handleOpenModelManager}>
        ...
        <div className="flex-1 overflow-y-auto p-2 space-y-2">
          <div className={`flex justify-between items-center px-2 py-1 ${styles.textMuted}`}>
            ...
            <button onClick={handleRefreshSessions} title="Refresh"><RefreshCcw size={12}/></button>
          </div>
          <PixelButton theme={theme} variant="primary" className="w-full text-xs mb-2" onClick={handleCreateNewSession}>
            + {t.newChat}
          </PixelButton>
      ...
      {isModelManagerOpen && (
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-sm">
          <Suspense fallback={<LoadingFallback message="Loading Model Manager..." />}>
            <ModelManager
              theme={theme}
              language={language}
              providers={providers}
